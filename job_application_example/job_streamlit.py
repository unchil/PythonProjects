...
airplane_url = "https://assets4.lottiefiles.com/packages/lf20_jhu1lqdz.json"
...
lottie_airplane = load_lottieurl(airplane_url)
...
with st.echo():
    @st.cache_data
    def load_airport_location():
        #Using Annotation @st.cache_data
        #Load necessary data
        return pd.read_csv('./job_application_example/airport_location.csv')


airport_distance_df = load_airport_location()
...
st.image('./job_application_example/haversine.png')
...
with st.echo():
    def get_distance_list(airport_df, airport_code):
        df = airport_df.copy()
        row = df[df.loc[:, "Airport Code"] == airport_code]
        lat = row["Lat"].values[0]
        long = row["Long"].values[0]
        ...
        df_to_return = df.sort_values(by="Distance(KM)").reset_index()
        return df_to_return

"""
To use this function, select an airport from the airports provided in the 
dataframe and this application will find the distance between each one, and 
...
st.write('Your closest airports information:')
...
df = pd.DataFrame.from_dict(example_row)

#st.write(df)

response = AgGrid(df, height=100, editable=True, )
...
if len(password_attempt) > 0:
    if password_attempt != df_edited['password'][0]:
        st.write(":red[Incorrect Password!]")
        st.stop()
    st.write('Congratulation! You are Pass the Test.')