...
endpoint_current = f'{BASE_URL}/sukub5mMaxDatetime/getSukub5mMaxDatetime'
url = f'{endpoint_current}?serviceKey={ActiveKey}'
...
if result.status_code == 200:
    try:
        ...
        if result_dict['response']['header']['resultCode'] == '00':
            item = result_dict['response']['body']['items']['item']
            df = pd.DataFrame(item, index=[0])
            df.to_sql(name=table_current, con=ENGIN, if_exists='append', index=False)
        else:
